					<div id="about4" class="block mB10">
					<h1 class="mB20"><?=$room[$id][name]?>教室のご紹介</h1>
						<h2><?=$room[$id][name]?>教室の教室情報</h2>
						<div class="section-lv2">
							<div class="shop-area clearfix">
								<div class="shop-box1-l">
                                    <dl> 
                                      <dt>教室名</dt> 
                                      <dd><?=$room[$id][name]?>教室</dd> 
                                    </dl> 
                                    <dl> 
                                      <dt>住所</dt> 
                                      <dd><?=$room[$id][ad1]?><br/><?=$room[$id][ad2]?><br /><?=$room[$id][ad3]?></dd> 
                                    </dl>
                                    <dl> 
                                      <dt>電話</dt> 
                                      <dd><?=$room[$id][phone]?></dd>
                                    </dl> 
                                    <dl> 
                                      <dt>開講時間</dt> 
                                      <dd><?=$room[$id][open]?></dd> 
                                    </dl>
                                    <dl> 
                                      <dt>アクセス</dt> 
                                      <dd><?=$room[$id][access]?></dd> 
                                    </dl>
									</div><!-- .shop-box1-l -->
									<div class="shop-box1-r"> 
									<div id="gmap" style="margin-bottom: 5px;width:270px; height:240px;"></div> 
									<p class="ali_right"><a href="http://maps.google.co.jp/maps?q=%E3%83%91%E3%82%BD%E3%82%B3%E3%83%B3%E5%B8%82%E6%B0%91%E8%AC%9B%E5%BA%A7<?=urlencode($room[$id][name])?>%E6%95%99%E5%AE%A4&ie=UTF-8&oe=utf-8" target="_blank">大きな地図でみる</a></p> 
									</div><!-- .shop-box1-r -->
								</div><!-- .shop-area -->
						</div><!-- /.section-lv2 -->
					</div><!-- /.block -->
				</div><!-- /.block -->
					<div>
						<div><img src="/<?=$room[$id][dir]?>/images/about_38.jpg" alt="教室の様子など" /></div>
<?php
if ($id == "6015097") {
?>
					<h2 class="mT25">教室からのメッセージ</h2>
						<p class="align-r mB20 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
							<p style="border:1px solid #cccccc; padding:20px;"> 
								パソコン市民講座　イトーヨーカドー明石教室のホームページへようこそ☆<br /><br />
								<!-- ◆仕事でExcelを使いこなせていないと思った事はありませんか？◆<br /><br />
								明石教室には、パソコンは初めて！という方から資格を目指される方、スマホ・タブレットを使いたい！など、さまざまな方が受講をされています。<br />
								その中でも最近、Excelでお困りの方が増えてきています。こんなケースありませんか？<br /><br />
								◆IF関数を使いたいけど、組み合わせ方とかわからない<br />
								◆VLOOKUP関数が使える様になりたい<br />
								◆ピボットを使う職場に異動になった・・・<br />
								◆マクロって何？　等々<br /><br />
								本当に使える様になるという事は、その機能を理解する事が一番大切です。<br />
								何となく出来たけど時間が掛かった、この手順でないと出来ない、ここまでなら出来るけど・・・<br />
								そう悩んだ事がある方は是非、一度教室までご相談ください。<br />
								1つの操作でも沢山の方法があります。この機会に効率よくエクセルが使える様になりませんか？<br /><br />
◆パソコンは初めて！ひとりで触るのが怖い…という方へ◆<br /><br />
パソコンの基礎を身に付けていない方は、どうしても「何か表示が出ると怖い・壊れちゃうかも」というイメージが強いのではないでしょうか？こんな事でお悩みの方も多いですよ。<br /><br />
◆メールやネット検索、文字入力もよくわからないから買ったのに使ってない…<br />
◆ご家族に聞いても教えてもらえない・・・嫌がられる<br />
・・・思い当たる事、ありませんか？<br />
明石教室では、そんなあなたの為に基礎の部分もしっかりサポート！<br />
復習可能の映像授業とインストラクターのサポートで「ひとりで不安」を「ひとりでも安心」へ、「怖い」を「楽しい★」に変えていきましょう！<br /><br />
明石教室は「自分で出来る事が楽しい☆自分に自信がついた！」そういう気持ちで通って頂ける教室です。<br />
パック受講ではなく、安定の定期受講スタイル。わからない事を聞くだけでなく、受講を通しパソコンを定期的に触り慣れることで「自分で出来るんだ！」という「自信」に繋げて頂けます。<br />
ご家族に聞きたくても聞きづらい事ってありますよね。<br />
この機会に、自分で出来る様になってご家族を驚かせてみませんか？<br /><br />
今、このページを開いて悩んでいるあなた、勇気を持って一度ご相談下さい☆<br /><br />
◆来年の年賀状は自分で作れるようになりませんか？◆<br /><br />
毎年、年賀状シーズンになるとパソコンを使う機会が増えてきますよね！<br />
この季節になるとこんな風に悩む方も意外と多いんです。思い当たる方は要チェック！<br /><br />
◆年賀状は家族に任せっきりで・・・<br />
◆作れないから毎年写真屋さんに頼んでる<br />
◆デザインは出来るけど宛名面は手書き<br /><br />
実は自分で年賀状を作ってみたい方、毎年作っているけどよく失敗している方、いらっしゃると思います。今から始めておけば、来年から簡単に素敵な年賀状が作成できる様になりますよ！<br />
ご自宅にパソコンをお持ちでしたら今からでも間に合います！来年からは、ご自分のオリジナル年賀状を作ってみませんか？<br />
自分で作る年賀状は、格別です♪<br /><br />
是非、トライしてみたい方は一度明石教室にご相談下さいね☆<br /><br /> -->
 ◆資格取得を考えていらっしゃる方へ◆<br /><br />
そろそろ仕事を探して働こう！と、色々お仕事を探していると・・・「ＰＣ操作が出来る方」「ワード・エクセルが出来る方」等、一文が入っている事が良くあります。最近では「～関数が出来る方」なんて特定のスキルを求めている会社も多いようです。<br /><br />
◆ＰＣ操作が出来るって、どれくらい？<br />
◆自分独学だから、本当の方法ってコレで正解なの？<br />
◆資格もないしアピールできるものが欲しいし、就職して活かしたい！<br />
◆自分が自信を持って提示出来る強みを作りたい<br /><br />
・・・等、就職・転職・再就職をお考えの方は考えてしまいますよね。<br />
そんなあなたには、資格対策講座がお勧めです。<br />
ただ資格を取得する内容だけの勉強ではなく、基盤としてのワード・エクセルの講座をご用意していますので、「経験ないし資格って無理じゃないかな・・・」と思っていらっしゃる方にも安心して受講いただけます。<br />
今のご自身のスキルが気になる方、今後に向けてスキルアップして資格をGETしませんか？<br />
就職活動をしながら、資格を目指して受講されている方も多数いらっしゃいます★<br /><br />
気になる方は、ご遠慮なく一度明石教室までご相談ください。<br /><br /> 
								　明石教室のインストラクター一同、心よりお待ちしています。
							</p> 
<?php
}elseif ($id == "6015002") {
?>
					<h2 class="mT25">教室からのメッセージ</h2>
						<p class="align-r mB20 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
							<p style="border:1px solid #cccccc; padding:20px;"> 
								パソコン市民講座　伊丹駅前教室のホームページへようこそ☆<br /><br />
								<!-- ◆仕事でExcelを使いこなせていないと思った事はありませんか？◆<br /><br />
								伊丹駅前教室には、パソコンは初めて！という方から資格を目指される方、スマホ・タブレットを使いたい！など、さまざまな方が受講をされています。<br />
								その中でも最近、IF関数を使いたい、VLOOKUP関数が使える様になりたい、ピボットを使う職場に異動になった・・・等々、Excelでお困りの方が増えてきています。<br />
								本当に使える様になるという事は、その機能を理解する事が大切です。<br />
								何となくできた、このやり方でないと出来ない、ここまでなら出来る・・・<br />
								と、思われた事がある方は是非、一度無料体験にお越しください。<br />
								一つの操作でも沢山のやり方があります。簡単な事で効率よくエクセルが使える事を体験で実感して下さいね。<br /><br />
								◆パソコンは初めて！触るのが怖い…という方◆<br /><br />
								インストラクターが親身にサポート、安心して「怖い」を「楽しい！」に変えていきましょう！<br />
								わからない事を聞くだけでなく、「自分の力で出来るんだ！」という「自信」に繋げて頂けます。<br />
								メールやネット、文字入力もわからない<br />
								それでも大丈夫！入力方法から「出来る」をサポート致します♪<br />
								ご家族に聞いても教えてもらえない<br />
								そうお困りの方、出来る様になってご家族を驚かせてみませんか？<br />
								「自分で出来るの事が楽しい☆自分に自信がついた！」そういう気持ちで通って頂ける教室です。<br />
								パック受講ではなく、安定の定期受講。自分のペースでスキルアップが可能なので安心です。<br />
								今、このページを開いて悩んでいるあなた、勇気を持って一度ご相談下さい☆<br /><br />
								 ◆就職や転職、職場でのパソコンスキルにお悩みの方◆<br /><br />
								教室に通えば、独学で「何となく…」が自信を持って「出来る」に変わり、すぐに仕事で活かせますよ♪<br />
								文字入力でお悩みの方、ビジネスシーンにはタイピングが必須！<br />
								タッチタイピングで「効率化」を実現しましょう！<br />
								今更「聞けないし…」そういった悩みも解消出来る<br />
								伊丹駅前教室はそんな方のスキルアップ道場です。<br />
								転職や就職を目指す、これから「変わりたい」という方は、<br />
								資格取得で自分の「武器」を手に入れましょう。<br />
								職場や周りの人に内緒で、こっそりスキルアップしてみませんか？<br />
								伊丹駅前教室のインストラクターが、全力であなたの「出来る」を応援します！<br /><br /> -->
								◆年賀状講座スタート！◆<br /><br />
								毎年、年賀状シーズンになるとパソコンを使う機会が増えてきます<br />
								宛名を手書きでしている方、自分で年賀状を作りたい方、毎年作っているけどよく失敗している方、来年の年賀状から、簡単に素敵な年賀状が作成できますよ！<br />
								ご自宅にパソコンがありましたら今からでも間に合います！ご自分のオリジナル年賀状を作ってみませんか？<br />
								自分で作る年賀状は、格別ですよ！<br />
								是非、伊丹駅前教室にご相談下さいね☆<br /><br /> 
								　伊丹駅前教室のインストラクター一同、心よりお待ちしています。
							</p> 
<?php
}elseif ($id == "6015121") {
?>
					<h2 class="mT25">教室からのメッセージ</h2>
						<p class="align-r mB20 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
							<p style="border:1px solid #cccccc; padding:20px;"> 
								パソコン市民講座　イオン西宮教室のホームページへようこそ☆<br /><br />
								<!-- ◆仕事でExcelを使いこなせていないと思った事はありませんか？◆<br /><br />
								イオン西宮教室には、パソコンは初めて！という方から資格を目指される方、スマホ・タブレットを使いたい！など、さまざまな方が受講をされています。<br />
								その中でも最近、IF関数を使いたい、VLOOKUP関数が使える様になりたい、ピボットを使う職場に異動になった・・・等々、Excelでお困りの方が増えてきています。<br />
								本当に使える様になるという事は、その機能を理解する事が大切です。<br />
								何となくできた、このやり方でないと出来ない、ここまでなら出来る・・・<br />
								と、思われた事がある方は是非、一度無料体験にお越しください。<br />
								一つの操作でも沢山のやり方があります。簡単な事で効率よくエクセルが使える事を体験で実感して下さいね。<br /><br />
								◆パソコンは初めて！触るのが怖い…という方◆<br /><br />
								インストラクターが親身にサポート、安心して「怖い」を「楽しい！」に変えていきましょう！<br />
								わからない事を聞くだけでなく、「自分の力で出来るんだ！」という「自信」に繋げて頂けます。<br />
								メールやネット、文字入力もわからない<br />
								それでも大丈夫！入力方法から「出来る」をサポート致します♪<br />
								ご家族に聞いても教えてもらえない<br />
								そうお困りの方、出来る様になってご家族を驚かせてみませんか？<br />
								「自分で出来るの事が楽しい☆自分に自信がついた！」そういう気持ちで通って頂ける教室です。<br />
								パック受講ではなく、安定の定期受講。自分のペースでスキルアップが可能なので安心です。<br />
								今、このページを開いて悩んでいるあなた、勇気を持って一度ご相談下さい☆<br /><br />
								 ◆年賀状講座スタート！◆<br /><br />
								毎年、年賀状シーズンになるとパソコンを使う機会が増えてきます<br />
								宛名を手書きでしている方、自分で年賀状を作りたい方、毎年作っているけどよく失敗している方、来年の年賀状から、簡単に素敵な年賀状が作成できますよ！<br />
								ご自宅にパソコンがありましたら今からでも間に合います！ご自分のオリジナル年賀状を作ってみませんか？<br />
								自分で作る年賀状は、格別ですよ！<br />
								是非、イオン西宮教室にご相談下さいね☆<br /><br /> -->
								◆スキルチェックを実施中！◆<br /><br />
								そろそろ仕事を探して働こう！と、色々お仕事を探していると・・・「ＰＣ操作が出来る方」「ワード・エクセルが出来る方」等、一文が入っている事が良くあります。<br />
								「ＰＣ操作が出来るって、どれくらい？」「自分は出来ているのかな？」等、ご自分のスキルを知りたい方は是非、教室の無料体験にお越しください。<br />
								基本操作、ワードのスキルチェック、エクセルのスキルチェックを体験で実施しております。<br /><br /> 
								　イオン西宮教室のインストラクター一同、心よりお待ちしています。
							</p> 
<?php
}elseif ($id == "6015104") {
?>
					<h2 class="mT25">教室からのメッセージ</h2>
						<p class="align-r mB20 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
							<p style="border:1px solid #cccccc; padding:20px;"> 
								パソコン市民講座　イトーヨーカドー甲子園教室のホームページへようこそ☆<br /><br />
<!-- ◆年賀状講座が始まります！◆<br /><br />
☆こんにちは！甲子園教室です☆<br />
今年も年賀状の季節が近づいてきましたね。<br /><br />
・宛名を手書きでしている方<br />
・今年こそ自分で年賀状を作りたい方<br />
・毎年作っているけど、よく失敗している方<br /><br />
甲子園教室へご相談ください！<br />
今年は満足のいく年賀状を、<strong>自分で作成</strong>しましょう！<br />
甲子園教室の年賀状講座なら、簡単に素敵な年賀状が作成できますよ！<br />
すでにパソコンをお持ちの方なら、今からでも間に合います！( ｀―´)ノ<br /><br />
まずは無料体験へ☆<br /><br />
◆『Excelを使いこなせていないかも！？(*_*)』と思った事はありませんか？◆<br /><br />
☆こんにちは！甲子園教室です☆<br />
最近、お仕事をしている方から『<strong>VLOOKUP関数が使える様になりたい</strong>』、部署異動や転職で『<strong>ピボットを使わないといけなくなった・・・</strong>』、『引き継いだ書類に設定されている<strong>関数のエラーが直せない(>_<)</strong>』等々、Excelでお困りだというお問い合わせが増えてきています。<br /><br />
<strong>本当に使える様になるという事は、その機能を理解する事が大切です。</strong><br /><br />
『何となくできた』<br />
『このやり方でないと、できない』<br />
『なんとかできるが時間がかかっている・・・』<br /><br />
そう思われた事がある方は、ぜひ一度無料体験にお越しください。<br />
一つの操作でも沢山のやり方があります。<br />
エクセルの『<strong>うまい使い方</strong>』伝授いたします！<br /><br />
今、このページを開いて悩んでいる方は、まず無料体験へ！<br /><br />
 ◆パソコンは初めて！触るのが怖い…という方◆<br /><br />
☆こんにちは！甲子園教室です☆<br />
パソコンを始めて触る方、ご安心ください！<br />
私たちがしっかりサポートいたします！<br /><br />
甲子園教室で練習すれば、あなたも必ず「怖い…」が「楽しい！」に変わります！<br /><br />
どうやったらネットで検索できる？文字入力もわからない・・・<br />
<strong>それでも大丈夫！</strong><br />
自信を持って『できる』ように、お手伝いいたします！！<br /><br />
家族や周りの人にはなかなか聞けない。聞いても教えてもらえない・・・(>_<)<br />
そうお困りの方、<strong>パソコン操作ができるようになって周りの方を驚かせてみませんか？</strong><br /><br />
『自分でできるって楽しい☆』<br /><br />
楽しく練習♪がモットーの明るい教室です！甲子園教室で、一緒にパソコンを楽しみましょう！<br /><br /> -->
◆きちんと『入力』できていますか？◆<br /><br />
☆こんにちは！甲子園教室です☆<br />
突然ですが、<br /><br />
『タッチタイピング』<br /><br />
できていますか？(`･ω･´)<br />
手元を見ずにキーボードのキーで入力することです。<br /><br />
『人差し指だけで十分入力できている！』というお声を聞くこともありますが、<strong>それではダメです！</strong><br /><br />
≪お仕事でパソコンを使う方≫<br />
文字入力は仕事効率に直結しています！<strong>スピーディーで正確なタイピング</strong>は作業時間の短縮に必須！！<br /><br />
≪趣味でパソコンを使う方≫<br />
ハガキや文書の作成からインターネットで調べもの、ブログ、メール・・・。文字入力はパソコンを使う上で切っても切り離せないものです！<br /><br />
どんな使い方をするときも、前を向いた、<strong>体に負担のかからない姿勢</strong>でパソコンに向かうことは肩こりや目の疲れを軽減させます。<br /><br />
甲子園教室ではタイピング練習を基礎からしっかり行います！<br />
小学生のお子さんから、80代のベテランの方まで、教室でしっかり入力されていますよ！<br />
入力のコツが知りたい方は、ぜひ無料体験へいらしてください！<br /><br /> 
								　甲子園教室のインストラクター一同、心よりお待ちしております(^^)/♪
							</p> 
<?php
}elseif ($id == "6015083") {
?>
					<h2 class="mT25">教室からのメッセージ</h2>
						<p class="align-r mB20 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
							<p style="border:1px solid #cccccc; padding:20px;"> 
								パソコン市民講座　イオン京橋教室のホームページへようこそ☆<br /><br />
								<!-- ◆仕事でExcelを使いこなせていないと思った事はありませんか？◆<br /><br />
								イオン京橋教室には、パソコンは初めて！という方から資格を目指される方、スマホ・タブレットを使いたい！など、さまざまな方が受講をされています。<br />
								その中でも最近、IF関数を使いたい、VLOOKUP関数が使える様になりたい、ピボットを使う職場に異動になった・・・等々、Excelでお困りの方が増えてきています。<br />
								本当に使える様になるという事は、その機能を理解する事が大切です。<br />
								何となくできた、このやり方でないと出来ない、ここまでなら出来る・・・<br />
								と、思われた事がある方は是非、一度無料体験にお越しください。<br />
								一つの操作でも沢山のやり方があります。簡単な事で効率よくエクセルが使える事を体験で実感して下さいね。<br /><br />
								◆パソコンは初めて！触るのが怖い…という方◆<br /><br />
								インストラクターが親身にサポート、安心して「怖い」を「楽しい！」に変えていきましょう！<br />
								わからない事を聞くだけでなく、「自分の力で出来るんだ！」という「自信」に繋げて頂けます。<br />
								メールやネット、文字入力もわからない<br />
								それでも大丈夫！入力方法から「出来る」をサポート致します♪<br />
								ご家族に聞いても教えてもらえない<br />
								そうお困りの方、出来る様になってご家族を驚かせてみませんか？<br />
								「自分で出来るの事が楽しい☆自分に自信がついた！」そういう気持ちで通って頂ける教室です。<br />
								パック受講ではなく、安定の定期受講。自分のペースでスキルアップが可能なので安心です。<br />
								今、このページを開いて悩んでいるあなた、勇気を持って一度ご相談下さい☆<br /><br />
								 ◆年賀状講座スタート！◆<br /><br />
								毎年、年賀状シーズンになるとパソコンを使う機会が増えてきます<br />
								宛名を手書きでしている方、自分で年賀状を作りたい方、毎年作っているけどよく失敗している方、来年の年賀状から、簡単に素敵な年賀状が作成できますよ！<br />
								ご自宅にパソコンがありましたら今からでも間に合います！ご自分のオリジナル年賀状を作ってみませんか？<br />
								自分で作る年賀状は、格別ですよ！<br />
								是非、イオン京橋教室にご相談下さいね☆<br /><br /> -->
								◆スキルチェックを実施中！◆<br /><br />
								そろそろ仕事を探して働こう！と、色々お仕事を探していると・・・「ＰＣ操作が出来る方」「ワード・エクセルが出来る方」等、一文が入っている事が良くあります。<br />
								「ＰＣ操作が出来るって、どれくらい？」「自分は出来ているのかな？」等、ご自分のスキルを知りたい方は是非、教室の無料体験にお越しください。<br />
								基本操作、ワードのスキルチェック、エクセルのスキルチェックを体験で実施しております。<br /><br /> 
								　イオン京橋教室のインストラクター一同、心よりお待ちしています。
							</p> 
<?php
}elseif ($id == "6015090") {
?>
					<h2 class="mT25">教室からのメッセージ</h2>
						<p class="align-r mB20 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
							<p style="border:1px solid #cccccc; padding:20px;"> 
								パソコン市民講座　イズミヤ東寝屋川教室のホームページへようこそ☆<br /><br />
								<!-- ◆仕事でExcelを使いこなせていないと思った事はありませんか？◆<br /><br />
								イズミヤ東寝屋川教室には、パソコンは初めて！という方から資格を目指される方、スマホ・タブレットを使いたい！など、さまざまな方が受講をされています。<br />
								その中でも最近、IF関数を使いたい、VLOOKUP関数が使える様になりたい、ピボットを使う職場に異動になった・・・等々、Excelでお困りの方が増えてきています。<br />
								本当に使える様になるという事は、その機能を理解する事が大切です。<br />
								何となくできた、このやり方でないと出来ない、ここまでなら出来る・・・<br />
								と、思われた事がある方は是非、一度無料体験にお越しください。<br />
								一つの操作でも沢山のやり方があります。簡単な事で効率よくエクセルが使える事を体験で実感して下さいね。<br /><br />
								◆パソコンは初めて！触るのが怖い…という方◆<br /><br />
								インストラクターが親身にサポート、安心して「怖い」を「楽しい！」に変えていきましょう！<br />
								わからない事を聞くだけでなく、「自分の力で出来るんだ！」という「自信」に繋げて頂けます。<br />
								メールやネット、文字入力もわからない<br />
								それでも大丈夫！入力方法から「出来る」をサポート致します♪<br />
								ご家族に聞いても教えてもらえない<br />
								そうお困りの方、出来る様になってご家族を驚かせてみませんか？<br />
								「自分で出来るの事が楽しい☆自分に自信がついた！」そういう気持ちで通って頂ける教室です。<br />
								パック受講ではなく、安定の定期受講。自分のペースでスキルアップが可能なので安心です。<br />
								今、このページを開いて悩んでいるあなた、勇気を持って一度ご相談下さい☆<br /><br />
								 ◆年賀状講座スタート！◆<br /><br />
								毎年、年賀状シーズンになるとパソコンを使う機会が増えてきます<br />
								宛名を手書きでしている方、自分で年賀状を作りたい方、毎年作っているけどよく失敗している方、来年の年賀状から、簡単に素敵な年賀状が作成できますよ！<br />
								ご自宅にパソコンがありましたら今からでも間に合います！ご自分のオリジナル年賀状を作ってみませんか？<br />
								自分で作る年賀状は、格別ですよ！<br />
								是非、イズミヤ東寝屋川教室にご相談下さいね☆<br /><br /> -->
								◆スキルチェックを実施中！◆<br /><br />
								そろそろ仕事を探して働こう！と、色々お仕事を探していると・・・「ＰＣ操作が出来る方」「ワード・エクセルが出来る方」等、一文が入っている事が良くあります。<br />
								「ＰＣ操作が出来るって、どれくらい？」「自分は出来ているのかな？」等、ご自分のスキルを知りたい方は是非、教室の無料体験にお越しください。<br />
								基本操作、ワードのスキルチェック、エクセルのスキルチェックを体験で実施しております。<br /><br /> 
								　イズミヤ東寝屋川教室のインストラクター一同、心よりお待ちしています。
							</p> 
<?php
}elseif ($id == "6015076") {
?>
					<h2 class="mT25">教室からのメッセージ</h2>
						<p class="align-r mB20 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
							<p style="border:1px solid #cccccc; padding:20px;"> 
								パソコン市民講座　ライフ香里園教室のホームページへようこそ☆<br /><br />
								<!-- ◆仕事でExcelを使いこなせていないと思った事はありませんか？◆<br /><br />
								ライフ香里園教室には、パソコンは初めて！という方から資格を目指される方、スマホ・タブレットを使いたい！など、さまざまな方が受講をされています。<br />
								その中でも最近、IF関数を使いたい、VLOOKUP関数が使える様になりたい、ピボットを使う職場に異動になった・・・等々、Excelでお困りの方が増えてきています。<br />
								本当に使える様になるという事は、その機能を理解する事が大切です。<br />
								何となくできた、このやり方でないと出来ない、ここまでなら出来る・・・<br />
								と、思われた事がある方は是非、一度無料体験にお越しください。<br />
								一つの操作でも沢山のやり方があります。簡単な事で効率よくエクセルが使える事を体験で実感して下さいね。<br /><br />
								◆パソコンは初めて！触るのが怖い…という方◆<br /><br />
								インストラクターが親身にサポート、安心して「怖い」を「楽しい！」に変えていきましょう！<br />
								わからない事を聞くだけでなく、「自分の力で出来るんだ！」という「自信」に繋げて頂けます。<br />
								メールやネット、文字入力もわからない<br />
								それでも大丈夫！入力方法から「出来る」をサポート致します♪<br />
								ご家族に聞いても教えてもらえない<br />
								そうお困りの方、出来る様になってご家族を驚かせてみませんか？<br />
								「自分で出来るの事が楽しい☆自分に自信がついた！」そういう気持ちで通って頂ける教室です。<br />
								パック受講ではなく、安定の定期受講。自分のペースでスキルアップが可能なので安心です。<br />
								今、このページを開いて悩んでいるあなた、勇気を持って一度ご相談下さい☆<br /><br />
								 ◆年賀状講座スタート！◆<br /><br />
								毎年、年賀状シーズンになるとパソコンを使う機会が増えてきます<br />
								宛名を手書きでしている方、自分で年賀状を作りたい方、毎年作っているけどよく失敗している方、来年の年賀状から、簡単に素敵な年賀状が作成できますよ！<br />
								ご自宅にパソコンがありましたら今からでも間に合います！ご自分のオリジナル年賀状を作ってみませんか？<br />
								自分で作る年賀状は、格別ですよ！<br />
								是非、ライフ香里園教室にご相談下さいね☆<br /><br /> -->
								◆スキルチェックを実施中！◆<br /><br />
								そろそろ仕事を探して働こう！と、色々お仕事を探していると・・・「ＰＣ操作が出来る方」「ワード・エクセルが出来る方」等、一文が入っている事が良くあります。<br />
								「ＰＣ操作が出来るって、どれくらい？」「自分は出来ているのかな？」等、ご自分のスキルを知りたい方は是非、教室の無料体験にお越しください。<br />
								基本操作、ワードのスキルチェック、エクセルのスキルチェックを体験で実施しております。<br /><br /> 
								　ライフ香里園教室のインストラクター一同、心よりお待ちしています。
							</p> 
<?php
}elseif ($id == "6015030") {
?>
					<h2 class="mT25">教室からのメッセージ</h2>
						<p class="align-r mB20 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
							<p style="border:1px solid #cccccc; padding:20px;"> 
								パソコン市民講座　ライフ門真教室のホームページへようこそ☆<br /><br />
								<!-- ◆仕事でExcelを使いこなせていないと思った事はありませんか？◆<br /><br />
								ライフ門真教室には、パソコンは初めて！という方から資格を目指される方、スマホ・タブレットを使いたい！など、さまざまな方が受講をされています。<br />
								その中でも最近、IF関数を使いたい、VLOOKUP関数が使える様になりたい、ピボットを使う職場に異動になった・・・等々、Excelでお困りの方が増えてきています。<br />
								本当に使える様になるという事は、その機能を理解する事が大切です。<br />
								何となくできた、このやり方でないと出来ない、ここまでなら出来る・・・<br />
								と、思われた事がある方は是非、一度無料体験にお越しください。<br />
								一つの操作でも沢山のやり方があります。簡単な事で効率よくエクセルが使える事を体験で実感して下さいね。<br /><br />
								◆パソコンは初めて！触るのが怖い…という方◆<br /><br />
								インストラクターが親身にサポート、安心して「怖い」を「楽しい！」に変えていきましょう！<br />
								わからない事を聞くだけでなく、「自分の力で出来るんだ！」という「自信」に繋げて頂けます。<br />
								メールやネット、文字入力もわからない<br />
								それでも大丈夫！入力方法から「出来る」をサポート致します♪<br />
								ご家族に聞いても教えてもらえない<br />
								そうお困りの方、出来る様になってご家族を驚かせてみませんか？<br />
								「自分で出来るの事が楽しい☆自分に自信がついた！」そういう気持ちで通って頂ける教室です。<br />
								パック受講ではなく、安定の定期受講。自分のペースでスキルアップが可能なので安心です。<br />
								今、このページを開いて悩んでいるあなた、勇気を持って一度ご相談下さい☆<br /><br />
								 ◆年賀状講座スタート！◆<br /><br />
								毎年、年賀状シーズンになるとパソコンを使う機会が増えてきます<br />
								宛名を手書きでしている方、自分で年賀状を作りたい方、毎年作っているけどよく失敗している方、来年の年賀状から、簡単に素敵な年賀状が作成できますよ！<br />
								ご自宅にパソコンがありましたら今からでも間に合います！ご自分のオリジナル年賀状を作ってみませんか？<br />
								自分で作る年賀状は、格別ですよ！<br />
								是非、ライフ門真教室にご相談下さいね☆<br /><br /> -->
								◆スキルチェックを実施中！◆<br /><br />
								そろそろ仕事を探して働こう！と、色々お仕事を探していると・・・「ＰＣ操作が出来る方」「ワード・エクセルが出来る方」等、一文が入っている事が良くあります。<br />
								「ＰＣ操作が出来るって、どれくらい？」「自分は出来ているのかな？」等、ご自分のスキルを知りたい方は是非、教室の無料体験にお越しください。<br />
								基本操作、ワードのスキルチェック、エクセルのスキルチェックを体験で実施しております。<br /><br /> 
								　ライフ門真教室のインストラクター一同、心よりお待ちしています。
							</p> 
<?php
} else {
?>
<?php
}
?>
						<div class="about_tel">
							<p><?=$room[$id][name]?>教室</p>
							<p class="phone">TEL:<?=$room[$id][phone]?></p>
						</div>
						<p class="align-r mB30 mR10"><a href="/<?=$room[$id][dir]?>/instructor.php">＞＞インストラクターの紹介はこちら</a></p>
						<p class="font18 mL20 mB10">パソコン市民講座が15年間の運営で延べ54万人以上の方に<br />「選ばれている理由」があります。<br />
					</div>
					<div id="about1" class="block mB30">
						<h2 class="mB20">パソコン市民講座の特長</h2>
						<div class="section-lv2 clearfix">
							<p class="font16 mL20 mB10">「全ての方にパソコンの楽しさを実感していただく」ための「安心」が、<br />あなたの学びをしっかりサポートします！</p>
							<h3 id="point1"><img src="/images/point_12.png" width="632" height="67" alt="①おひとりお一人のペースで学べる「個別学習」" /></h3>
							<div class="section-lv3 pt1">
								<p class="mB10">
								パソコン市民講座の「個別学習」は初心者の方も、資格取得を目指す方も自分のペースで学習いただけます。分かりやすい映像授業に加え、教室インストラクターが皆さんの質問に丁寧にお答えします。おひとりお一人の学習進度や理解度を確認し、自立してパソコンを使いこなしていただくための各種サポートを行います。
								</p>
							</div>

							<h3 id="point2"><img src="/images/point_15.png" width="632" height="67" alt="②曜日と時間が自由に選べる「受講予約システム」" /></h3>
							<div class="section-lv3 pt3">
								<p class="mB10">
								予定は毎月変わるもの。受講する曜日・時間帯は、教室の開講時間内で自由に設定いただけます。また教室はショッピングセンター内にあるので、買い物ついでや仕事帰りなど、慣れ親しんだ通いやすい環境です。ご自分のライフスタイルに合わせて無理なく学べる受システムにご好評をいただいております。
								</p>
							</div>

							<h3 id="point3"><img src="/images/point_18.png" width="632" height="67" alt="③学びやすく続けやすい安心の「月謝制」（1時間1500円）" /></h3>
							<div class="section-lv3 pt2">
								<p class="mB10">
								受講料は、1時間あたり1,500円（税別）の毎月定額支払制となっていますので、計画的に学習を進めることができます。また800時間以上の豊富な講座の中から、インストラクターが個々に合わせたオリジナルカリキュラムをご提案いたします。パソコン市民講座はお一人でパソコン等の操作ができるようになる「自立」にこだわるパソコン教室です。<br /> （入会金、月会費、テキスト教材費が別途必要です）
							</p>
							</div>
							
							<h3 id="point4"><img src="/images/point_21.png" width="632" height="67" alt="④自宅でも復習ができる「映像配信システム」" /></h3>
							<div class="section-lv3 pt5">
								<p class="mB10">
								パソコンスクール業界初！教室でご覧いただく映像教材が、ご自宅でも視聴できる「映像配信サービス」を2015年4月から導入！<br />
                                教室で受講いただいた講座を専用の学習サイトから、この「映像配信サービス」を利用して、自宅や外出先で繰り返し復習いただけます。<br />
                                「映像配信サービス」を利用すれば、学習内容をしっかりと身に付けることができます！

								</p>
								<div class="mB10 mL5"><a href="http://www.4353p-club.com" target="_blank"><img class="alpha" src="/images/premiere04.jpg" alt="プレミア倶楽部ウェブサイトはこちら" /></a></div>
							</div>

							<h3 id="point5"><img src="/images/point_24.png" width="632" height="67" alt="⑤全国150教室、15年間で54万人の「指導実績」" /></h3>
							<div class="section-lv3 pt4">
								<p class="mB10">
								54万人以上の方々への「豊富な指導実績」を誇ります！あらゆる世代の方々にITスキルを身に付けていただき、便利で豊かな生活を送っていただくことを目指しております。<br />
                                これまで蓄積してきたノウハウをもとに、パソコン機器等を自立して操作できる方をお一人でも多く輩出し、これからも「できた！」「楽しい！」という喜びの声を増やしてまいります。
								</p>
							</div>

							<h3 id="point6"><img src="/images/point_25.png" width="632" height="67" alt="⑥ご自宅でのトラブルも無料で対応「電話サポート」" /></h3>
							<div class="section-lv3 pt6">
								<p class="mB10">
								パソコン市民講座の受講生の皆様には、ご自宅でのトラブルにも無料で電話対応を行う「パソコンサポートサービス」をご利用いただくことができます。<br />パソコンやiPad・iPhoneに関するトラブル、周辺機器のトラブルにも対応いたします。機器関連でわからないことがあった場合でもご自宅から専用の電話番号へお電話いただければ、専門のスタッフが安心のサポートを行います。
								</p>
							</div>
							
						</div><!-- /.section-lv2 -->
						<div class="back-top mR15 mB20"><a class="moz2" href="#header">ページトップへ</a></div>
					</div><!-- /.block -->
<?php
if ($id == "6117001") {
?>
<?php
} else {
?>
					<div id="contact2" class="block mB30">
						<h2 class="mB10">興味を持ったらまずはお問い合わせ</h2>
						<div class="section-lv2 clearfix moz2">
							<div class="box-a">
								<p><img src="/images/contact_53.png" width="350" height="24" alt="お問い合わせフォームはこちら" /></p>
								<div class="section-lv3 moz2">
									<img src="/images/contact_60.png" width="94" height="68" alt="お気軽にお問い合わせください。" /><span class="contact-bt"><a href="/<?=$room[$id][dir]?>/contact.php" onclick="javascript:_gaq.push(['_trackPageview', '/click/contact/2/']);"><img class="alpha" src="/images/contact_61.png" width="250" height="71" alt="お問い合わせ" /></a></span>
								</div>
							</div>
							<div class="box-b">
								<p><img src="/images/contact_55.png" height="24" width="260" alt="お電話での問い合わせはこちら"  /></p>
								<div class="section-lv3 moz2">
									<dl class="b mB10">
										<dt class="font10">ホームページを見たとお伝えください！</dt>
										<dd><span class="font22 blue"><?=$room[$id][phone]?></span></dd>
									</dl>
									<p><span class="font12">営業時間は<a href="/<?=$room[$id][dir]?>/about.php#about4">こちらをご覧ください</a></span></p>
								</div>
							</div>
						</div><!-- /.section-lv2 -->
					</div><!-- /#.block -->
<?php
}
?>
					
					<div id="about2" class="block mB30 mT25">
						<h2 class="mB20">イベントカレンダー 2017年版</h2>
						<div class="section-lv2 clearfix">
							<h3 class="mB10"><img src="/images/about_25.png" width="632" height="30" alt="さらに…「大人気！」教室イベントを毎月開催！" /></h3>
							<p class="mL20 mR20 mB10">パソコン市民講座では毎月「教室での楽しいイベント」を開催しています。
							<br />日頃は個別で学ぶシステムですが、イベントでは会員の方が集まりワイワイと交流しながら、パソコンでのモノ作りやコンテストに皆さんで一緒にチャレンジしていきます。<br />他の会員の方との楽しい交流の輪が広がり、「自分にもこんなモノが作れた！」という達成感を実感できる！そんな大人気の教室イベントにあなたも参加してみませんか？
							</p>
							<div class="mL20">
								<img class="mL20" src="/images/about_27.jpg" width="543" alt="イベントカレンダー" />
							</div>
							<div class="back-top mR15"><a class="moz2" href="#header">ページトップへ</a></div>
						</div><!-- /.section-lv2 -->
					</div><!-- /.block -->

					<div id="about3" class="block mB30">
					<h2 class="mB20">受講システム</h2>
					<div class="section-lv2 clearfix">
					<table class="table1 mB20">
					<tr>
					<td class="item">受講料</td>
					<td>１時間あたり 1,500円（税別）<br />
						※毎月定額支払制となっていますので、計画的に学習を進めることができます。<br />
						※パソコン等、機器使用料は不要です。</td>
					</tr>
					<tr>
					<td class="item">その他費用</td>
					<td>入会金（初回のみ）、月会費（毎月）、テキスト教材費が必要です。</td>
					</tr>
					<tr>
					<td class="item">受講ペース</td>
					<td>「1回2時間、週2～3日」が標準ペースです。<br />
					※短期間でマスターできる集中受講も可能です。</td>
					</tr>
					</table>
					</div><!-- /.section-lv2 -->
					</div><!-- /.block -->
				</div><!-- /.block -->
				<div class="back-top mR15"><a class="moz2" href="#header">ページトップへ</a></div>

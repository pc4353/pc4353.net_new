	...
	function render_page( ) {
		...
		if ( isset( $_POST['update'] ) && check_admin_referer( 'siteguard-menu-admin-filter-submit' ) ) {
			...
			if ( false == $error ) {
				...
				$opt_val_exclude = stripslashes( $_POST[ $opt_name_exclude ] );
				$config->set( $opt_name_feature, $opt_val_feature );
				$config->set( $opt_name_exclude, $this->cvt_ret2camma( $opt_val_exclude ) );
				$config->update( );
				$result = true;
				if ( '0' == $opt_val_feature ) {
					$result = $admin_filter->feature_off( );
				} else {
					$result = $admin_filter->feature_on( $_SERVER['REMOTE_ADDR'] );
				}
				if ( true === $result ) {
